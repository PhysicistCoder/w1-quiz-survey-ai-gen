...
class GenerateExamPage(Page):
    ...
    description = """
    This app, powered by GPT-3.5 (ChatGPT), is part of a workshop to learn about AI assistants. 
    It can generate exam questions and answers, as well as survey questions on various topics. 
    Participants can practice and test their knowledge by downloading the questions as a PDF file or taking the exam within the app. 
    The focus is on exploring the capabilities of AI assistants and understanding how they can be used for educational purposes in generating exam or survey content.
    """
    ...
    def render(self, app):
        ...
        # input parameters for openai key
        openai_key = st.text_input(
            "OpenAI key",
            placeholder="OpenAI key",
            help="OpenAI key for the API"
        )
        ...
        if st.button("Generate", help="Generate the questions according to the parameters"):
            ...
            try:
                ...
                st.error("An error occurred while generating the questions. Please try again" + str(e))

        if app.questions is not None:

            st.info(
                f"An exam/survey with {len(app.questions)} questions has been generated. You "
            ...
            with right:
                if st.button("Start exam (If not survey!)", help="Start the exam"):
...
class ResultsPage:
    ...
    def render(self, app):
        ...
        with left:
            ...
            if st.button("Generate new exam/survey"):
                app.reset()
