...
def prepare_prompt(topics: str, number_of_questions: int, number_of_answers: int) -> str:
    ...
    """
    return (
        f"Create an exam or survey of multiple choice questions with {number_of_questions} "
        f"questions and {number_of_answers} of possible answers in each question. "
        f"Put the correct answer in bold (surrounded by **) in its original spot if exam. "
        f"The exam should be about {topics}, or if the topic mentions survey then it should be survey focused. Only generate the questions and "
...
def response_to_questions(response: str) -> List[Question]:
    ...
    for question_text in response.split("\n\n"):
        ...
        try:
            ...
            print("answers[correct_answer]",answers[correct_answer])
        except:
            print("Error in response_to_questions")
